...
void ProduceNextGen()
{
	...
	int foodsMin = pData->nbFoodFix * (1 + ((float) pData->foodVariance / 100.f));
	int foodsMax = pData->nbFoodFix * (1 - ((float) pData->foodVariance / 100.f));
	pData->nbFood = GetRandomValue(foodsMin, foodsMax);
